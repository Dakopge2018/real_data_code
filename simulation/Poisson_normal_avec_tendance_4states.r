...
generate_trig_covariates <- function(time, period, degree) {
  trig_covs <- data.frame(time = time, trend = floor(time/365))
  for (d in 1:degree) {
...
simulate_observations <- function(states, n, period, mu, nu, delta_poisson, delta_normal, sd, degree, trend_params) {
  ...
  for (t in 1:n) {
    ...
    # Poisson
    lambda_t <- exp(mu[k] + trend_params[k] * floor(t/365)) * exp(sum(delta_poisson[k, ] * c(sin(2 * pi * (1:degree) * floor(t/365) / period), cos(2 * pi * (1:degree) * floor(t/365) / period))))
    obs_poisson[t] <- rpois(1, lambda = lambda_t)
    
    # Normal
    mean_t <- nu[k] + trend_params[k] * floor(t/365) + sum(delta_normal[k, ] * c(sin(2 * pi * (1:degree) * floor(t/365) / period), cos(2 * pi * (1:degree) * floor(t/365) / period)))
