...
estimer_simuler_hmm <- function(donnees, nb_etats = 2, n_simulations = 1, 
                               ...
                               plot_results = TRUE, n_ajustements = 40) {
...
# ...existing code...
  ...
  for(i in 1:n_simulations) {
    sim_data <- simulate(modele_ajuste, nsim = 1000)
  ...
  if(plot_results) {
    ...
    png(file.path(OUTER_IMAGE, "visualization1.png"), width=1500, height=1000)
    grid.arrange(p1, p2, p3,p3_bis, ncol = 1)
    dev.off()
    pdf(file.path(OUTER_IMAGE, "visualization1.pdf"), width=8, height=6)
    grid.arrange(p1, p2, p3, p3_bis, ncol = 1)
    dev.off()
    ...
    dev.off()
    pdf(file.path(OUTER_IMAGE, "visualization2.pdf"), width=8, height=6)
    grid.arrange(p4, p5, p6, ncol = 1)
    dev.off()
    ...
    dev.off()
    pdf(file.path(OUTER_IMAGE, "visualization3.pdf"), width=10, height=8)
    grid.arrange(p7, p8, ncol = 2)
    dev.off()
    ...
    dev.off()
    pdf(file.path(OUTER_IMAGE, "visualization4.pdf"), width=10, height=8)
    grid.arrange(p9, p10, ncol = 2)
    dev.off()
